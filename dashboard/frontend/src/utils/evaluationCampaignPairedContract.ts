...
export function decodeEvaluationCampaignPairedLiveEvidence(
  ...
  const statistics = value.statistics.map((statistic) =>
  ...
  const hasModelPool = trackIDs.includes('model_pool')
  if (!hasModelPool) {
  ...
  const armReliability = value.model_pool_arm_reliability.map((statistic) =>
  ...
  return {
    ...(value as unknown as EvaluationCampaignPairedLiveEvidence),
