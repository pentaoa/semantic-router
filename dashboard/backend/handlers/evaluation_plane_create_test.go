...
func assertLiveCapacityCreateSuccess(t *testing.T, handler *EvaluationPlaneHandler, targetID string) {
	t.Helper()
	response := httptest.NewRecorder()
...
func TestEvaluationPlaneCreateRequiresEveryWireField(t *testing.T) {
	...
	if len(runs) != 0 {
		t.Fatalf("rejected wire requests created %d runs", len(runs))
	}
}
