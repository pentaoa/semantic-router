...
func TestResolveImportedSuiteEvidenceIsE0AndQualifiesNoGate(t *testing.T) {
	...
	firstRevision := writeImportedSuiteFixture(t, service.suiteStorePath, "import-a")
	secondRevision := writeImportedSuiteFixture(t, service.suiteStorePath, "import-b")
	...
	qualification, err := resolveSuiteGateQualificationForTest(t, service.suiteStorePath, manifest)
	if err != nil {
...
func TestResolveImportedSuiteRejectsMissingMixedChangedAndCorruptEvidence(t *testing.T) {
	t.Run("missing private suite", func(t *testing.T) {
		...
		_, err := resolveSuiteGateQualificationForTest(t, service.suiteStorePath, manifest)
	...
	t.Run("mixed executor families", func(t *testing.T) {
		service, _ := newTestService(t, &controlledProcess{}, 1)
		revision := writeImportedSuiteFixture(t, service.suiteStorePath, "imported")
	...
	t.Run("frozen revision changed", func(t *testing.T) {
		service, _ := newTestService(t, &controlledProcess{}, 1)
		writeImportedSuiteFixture(t, service.suiteStorePath, "imported")
	...
	t.Run("manifest content tampered", func(t *testing.T) {
		...
		revision := writeImportedSuiteFixture(t, service.suiteStorePath, "imported")
		manifest := normalizedReplayManifest("imported", revision)
		indexBytes, readErr := os.ReadFile(filepath.Join(service.suiteStorePath, "index", "imported.json"))
		if readErr != nil {
			t.Fatal(readErr)
		}
		...
		path := filepath.Join(service.suiteStorePath, "manifests", "sha256", strings.TrimPrefix(index.ManifestDigest, "sha256:"))
...
func TestResolveImportedSuiteRejectsForgedPromotionAndNativeRunClaims(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			service, _ := newTestService(t, &controlledProcess{}, 1)
			revision := writeImportedSuiteFixture(t, service.suiteStorePath, "forged", test.options)
			_, err := resolveSuiteGateQualificationForTest(t, service.suiteStorePath, normalizedReplayManifest("forged", revision))
...
func TestImportedSuiteNeverPromotesLiveExecution(t *testing.T) {
	service, _ := newTestService(t, &controlledProcess{}, 1)
	revision := writeImportedSuiteFixture(t, service.suiteStorePath, "live-suite")
