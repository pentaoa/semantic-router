...
func validateRecoveryGate(gate Gate, records recordAttestation) error {
	...
	if method.Passed == nil || method.PassRateLower95 == nil {
		return requireUnavailableGate(gate, "a complete server-brokered live exact-step fault window with the platform minimum cohort")
	}
	verdict := GateVerdict("fail")
	if *method.Passed {
		verdict = "pass"
	}
	return requireGateDecision(gate, verdict, *method.PassRateLower95, GateThreshold{
