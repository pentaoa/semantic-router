...
func validateCampaignDecisionContract(campaign Campaign) error {
	...
	if !decision.CreatedAt.Equal(campaign.CreatedAt) || decision.Summary == "" {
		return fmt.Errorf("%w: evaluation campaign decision time or summary is invalid", ErrInvalid)
	}
...
func validateStoredCampaignGate(profile ChangeProfile, index int, gate CampaignGate) error {
	...
	expectedName, disposition, minimum := gateNames[index], "required", EvidenceLevel("E5")
	...
	if gate.Disposition == "not_applicable" {
		if gate.Verdict != "not_applicable" || gate.Observed != nil || gate.Threshold != nil || len(gate.EvidenceRefs) != 0 {
