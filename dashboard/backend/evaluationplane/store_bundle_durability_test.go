...
func TestCreateBundleConflictPreservesPublishedBundle(t *testing.T) {
	...
	if _, err := service.store.CreateBundle(run, manifest); !errors.Is(err, ErrConflict) {
...
func TestCreateBundlePublishesCompleteInitialSnapshot(t *testing.T) {
	...
	manifestPath, err := service.store.CreateBundle(run, manifest)
	if err != nil {
...
func preparePendingRun(t *testing.T, service *Service, request CreateRunRequest) (Run, RunManifest) {
	...
	run, manifest, err := service.newPendingRunManifest(
		registry, validated, target, snapshot, evidenceLevel,
	)
	if err != nil {
