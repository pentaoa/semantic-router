...
func TestDecisionAlgorithmNameMatchesExtProcRuntimeSelectionProjection(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "unknown falls back to static", decision: routerconfig.Decision{ModelRefs: models, Algorithm: &routerconfig.AlgorithmConfig{Type: "unknown"}}, want: "static"},
		{name: "known selector", decision: routerconfig.Decision{ModelRefs: models, Algorithm: &routerconfig.AlgorithmConfig{Type: "router_dc"}}, want: "router_dc"},
		{name: "fast response", decision: fastResponse, want: "fast_response"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if got := decisionAlgorithmName(test.decision); got != test.want {
