...
func campaignTestPairedGate(id string, evidence CampaignPairedLiveEvidence, fixture campaignPairedLiveFixture) CampaignGate {
	index := 0
	for requiredGateIDs[index] != id {
		index++
	}
	base := CampaignGate{
		ID: id, Name: gateNames[index], Disposition: "required", Verdict: "unavailable",
		EvidenceLevel: gateEvidenceLevels[index], EvidenceRefs: []string{}, Rationale: "missing",
	}
