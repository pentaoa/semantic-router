	...
	for index := 2; index < 10; index++ {
		for _, verdict := range []GateVerdict{"pass", "fail"} {
			t.Run(reportGateTestName(index, verdict), func(t *testing.T) {
				...
				case 6:
					report.Metrics = []Metric{{ID: "agentic.recovery_pass_rate_lower_95", Value: &observed}}
				case 7:
...
func TestCapacityGateUsesOnlyServerReducedSLOHeadroom(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			records.Metrics = recordMetricAttestation{
				CapacitySuccessRate:  reducedMetricEvidence{Value: float64Pointer(1), SampleCount: 2},
				CapacityRowsByCase:   map[string]int{"case-1": 2},
				CapacityLevelsByCase: map[string]map[int64]struct{}{"case-1": {1: {}, 2: {}}},
			}
			attestation := &capacitySLOAttestation{Headroom: test.headroom, LevelCount: 2}
			...
			records.Metrics.CapacitySuccessRate.SampleCount = 3
...
func TestQualifiedReceiptCannotReplaceMissingTypedGateReducers(t *testing.T) {
	for _, test := range []struct {
		...
		{gateID: "G6", trackID: "agentic", index: 6, observed: 1, threshold: GateThreshold{Operator: ">=", Value: minimumRecoveryPassRateLowerBound, Unit: "fraction"}, metricID: "agentic.recovery_pass_rate_lower_95", missing: "fault window"},
