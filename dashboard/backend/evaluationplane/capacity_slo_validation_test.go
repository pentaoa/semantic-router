...
func TestCreateRunRequiresCurrentCapacityContract(t *testing.T) {
	...
	if _, err := service.CreateRun(context.Background(), replay); !errors.Is(err, ErrInvalid) ||
...
func TestCreateRunFreezesCapacityContractIntoRunAndManifest(t *testing.T) {
	...
	if _, retryErr := service.CreateRun(context.Background(), changed); !errors.Is(retryErr, ErrConflict) {
