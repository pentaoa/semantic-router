...
func writeDeclaredShiftSuiteSource(t *testing.T) declaredShiftLiveFixtureSource {
	...
	revision := writeImportedSuiteFixture(t, service.suiteStorePath, "declared-shift", importedSuiteFixtureOptions{
	...
	document, err := loadInstalledSuiteDocument(service.suiteStorePath, "declared-shift")
	if err != nil {
		t.Fatal(err)
	}
...
func writeDeclaredShiftLiveFixture(t *testing.T, targetArmID string) declaredShiftLiveFixture {
	...
	sourceRecord["evidence_kind"] = declaredShiftLiveEvidenceKind
	...
	targetRecord["evidence_kind"] = declaredShiftLiveEvidenceKind
	...
	if writeLineageErr := writeJSONAtomic(filepath.Join(runDir, "lineage.json"), map[string]any{
		"resolved_snapshot":           map[string]any{"schema_version": SchemaVersion, "manifest_digest": manifest.ManifestDigest},
	...
	qualification, err := resolveSuiteGateQualification(
		service.suiteStorePath, manifest, builtinExecutorContractForTest(t, normalizedSuiteLiveExecutorID),
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestLiveDeclaredShiftRejectsForgedInstalledRelationBindings(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "forged evidence level", mutate: func(rows []map[string]any) {
			rows[1]["evidence_kind"] = "qualified-live.v1;level=E4"
		}},
...
func TestReplayCannotClaimServerLiveDeclaredShiftMethod(t *testing.T) {
	...
	kind := declaredShiftLiveEvidenceKind
	success := true
...
func TestImportedNormalizedSuiteCannotQualifyLiveDeclaredShift(t *testing.T) {
	...
	revision := writeImportedSuiteFixture(t, service.suiteStorePath, "unverified-import")
