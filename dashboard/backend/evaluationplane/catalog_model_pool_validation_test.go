...
func TestLiveModelPoolCandidateSeparatesPolicyFromArmComposition(t *testing.T) {
	...
	if err := service.validateComparableTargetSnapshot("model_pool", target, snapshot, baseline.ID); !errors.Is(err, ErrInvalid) || !strings.Contains(err.Error(), "runtime origins") {
