...
func writeStagingTestLineage(t *testing.T, path string) {
	...
	if err := writeJSONAtomic(path, map[string]any{
		...
		"discovered_entrypoints": []any{}, "executors": []any{},
	}); err != nil {
...
func TestBoundedConcurrencyAndSuccessfulExitRequiresReport(t *testing.T) {
	...
	if _, err := service.StartRun(context.Background(), second.ID); !errors.Is(err, ErrConflict) {
	...
	queued, queuedErr := service.GetRun(second.ID)
	...
	noReportRun, noReportErr := noReportService.CreateRun(context.Background(), validCreateRequest())
	if noReportErr != nil {
		t.Fatalf("create no-report run: %v", noReportErr)
	}
	if _, err := noReportService.StartRun(context.Background(), noReportRun.ID); err != nil {
...
func TestServiceCloseCancelsAndWaitsForWorkerExit(t *testing.T) {
	...
	pending, pendingErr := service.CreateRun(context.Background(), validCreateRequest())
	if pendingErr != nil {
		t.Fatalf("CreateRun after Close: %v", pendingErr)
	}
	if _, err := service.StartRun(context.Background(), pending.ID); !errors.Is(err, ErrConflict) {
		t.Fatalf("StartRun after Close error=%v, want ErrConflict", err)
	}
	if _, _, err := service.Subscribe(pending.ID); !errors.Is(err, ErrConflict) {
...
func TestWorkerTimeoutFailsRunAndReleasesCapacity(t *testing.T) {
	...
	second, secondErr := service.CreateRun(context.Background(), validCreateRequest())
	if secondErr != nil {
		t.Fatalf("create second: %v", secondErr)
	}
	startRunAfterCapacityRelease(t, service, second.ID)
...
func startRunAfterCapacityRelease(t *testing.T, service *Service, runID string) {
	...
	for {
		_, err := service.StartRun(context.Background(), runID)
		if err == nil {
			return
		}
...
func TestStoreRejectsSharedDirectoryPermissions(t *testing.T) {
	...
	if _, err := NewStore(nestedRoot); err == nil {
