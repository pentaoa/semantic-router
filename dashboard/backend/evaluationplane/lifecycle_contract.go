...
const (
	lifecyclePolicySchemaVersion = "evaluation-lifecycle-policy.v1"
	runLifecycleSchemaVersion    = "evaluation-run-lifecycle.v1"
	lifecycleAuditSchemaVersion  = "evaluation-lifecycle-audit.v1"
	lifecyclePolicyRevision      = "evaluation-lifecycle-policy.2026-08-31"
...
func normalizeLifecycleLimits(limits LifecycleLimits) (LifecycleLimits, error) {
	...
	if limits.MaxOwnerBytes < reservedRunBytes || limits.MaxStoreBytes < reservedRunBytes ||
		limits.MaxOwnerBytes > limits.MaxStoreBytes || limits.MaxOwnerRuns < 1 ||
		limits.MaxOwnerRuns > 100_000 || limits.MaxAuditBytes < maxLifecycleRecordSize ||
...
type UpdateRunLifecycleRequest struct {
