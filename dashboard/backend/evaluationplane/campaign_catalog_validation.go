...
func validateCampaignCatalogContracts(registry *Registry, profiles []CatalogChangeProfile) error {
	for _, profile := range profiles {
		if len(profile.CampaignSlots) != len(requiredGateIDs)-2 {
			return fmt.Errorf("campaign profile %q must define G2-G9 exactly once", profile.ID)
		}
		seen := make(map[string]bool, len(profile.CampaignSlots))
		for index, slot := range profile.CampaignSlots {
			expectedGateID := requiredGateIDs[index+2]
			...
			if slot.Disposition != "required" && slot.Disposition != "advisory" &&
				slot.Disposition != "not_applicable" {
			...
			if slot.GateID == "G3" && slot.Disposition == "required" &&
...
func campaignBindingKindForGate(gateID string) CampaignBindingKind {
	switch gateID {
	case "G3":
		return CampaignBindingControlledPair
	case "G5":
		return CampaignBindingFidelityPair
	default:
		return CampaignBindingRun
	}
}
