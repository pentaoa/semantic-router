"""Executable normalization inventory for all audited benchmark adapters."""
...
def _descriptor(
    ...
    return BenchmarkNormalizerDescriptor(
...
def get_benchmark_normalizer(adapter_id: str) -> BenchmarkNormalizerDescriptor:
    for descriptor in _NORMALIZERS:
        if descriptor.adapter_id == adapter_id:
            return descriptor
    raise ValueError(f"unknown benchmark normalizer: {adapter_id}")
