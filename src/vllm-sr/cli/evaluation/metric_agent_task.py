...
from cli.evaluation.agent_task_evidence import AgentTaskMethodEvidence, _float_equal
from cli.evaluation.evidence import ExecutionRecord
from cli.evaluation.metric_core import _metric
from cli.evaluation.metric_recovery import _one_sided_wilson_lower
...
class _AgentTaskAccumulator:
    ...
    def build(self) -> AgentTaskReduction:
        ...
        return AgentTaskReduction(
            ...
            task_success_rate_lower_95=_one_sided_wilson_lower(self.successful, count),
            task_reliability=reliable / distinct_tasks,
            task_reliability_lower_95=_one_sided_wilson_lower(reliable, distinct_tasks),
...
def agent_task_metrics(records: list[ExecutionRecord]) -> list[EvaluationMetric]:
    reduced = reduce_agent_tasks(records)
    values = _task_outcome_metric_values(reduced) + _task_policy_metric_values(reduced)
    return [
        _metric(metric_id, name, "agentic", value, unit, direction, count)
