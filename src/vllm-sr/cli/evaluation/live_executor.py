...
def _route_records(
    ...
    def evaluate(case: CaseVisible) -> tuple[ExecutionRecord, RoutingDiagnostic | None]:
        ...
        selected = _arm_for_selector(diagnostic.selected_model, mixture.model_arms)
        if (
            result.success
    ...
    def evaluate(
        ...
        return (
            ExecutionRecord(
                ...
                evidence_kind="live-mom-arm-outcome.v1",
                broker_receipt=result.broker_receipt,
                error=result.error,
            ),
            result,
    ...
    def evaluate(case: CaseVisible) -> tuple[ExecutionRecord, HTTPResult]:
        ...
        return (
            ExecutionRecord(
                ...
                output_tokens=output_tokens,
                runtime_cost=runtime_cost,
                evidence_kind="live-mom-routed-outcome.v1",
                broker_receipt=result.broker_receipt,
                error=result.error,
            ),
            result,
