...
def evaluate_g6(
    ...
    observed = context.recovery_pass_rate_lower_bound
    threshold = context.recovery_minimum_pass_rate_lower_bound
    if context.trajectory_qualified is None or observed is None or threshold is None:
        return build_gate(
            definition,
            disposition,
            metadata,
    ...
    passed = context.trajectory_qualified
    return build_gate(
        definition,
        disposition,
        metadata,
        ...
        rationale=(
            "The complete repeated-seed live fault window met exact injection, state, retry, latency, side-effect, and one-sided 95% recovery reliability requirements."
            if passed
            else "The complete repeated-seed live fault window failed an exact recovery requirement or its one-sided 95% reliability lower bound."
        ),
    )
