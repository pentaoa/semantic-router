...
def materialize_evaluation_deployment_snapshot(
    ...
    if destination.exists() or destination.is_symlink():
        _verify_existing_snapshot(destination, files)
        return destination
    ...
    try:
        for relative, content in files.items():
            ...
            try:
                ...
                os.close(descriptor)
        _freeze_snapshot_permissions(temporary)
        _verify_existing_snapshot(temporary, files)
...
def _freeze_snapshot_permissions(root: Path) -> None:
    for directory, _, filenames in os.walk(root, topdown=False, followlinks=False):
        ...
        for filename in filenames:
            ...
            if path.is_symlink() or not path.is_file():
                raise ValueError(
                    "Evaluation deployment snapshot contains an unsafe file"
                )
            path.chmod(_SNAPSHOT_FILE_MODE)
        current.chmod(_SNAPSHOT_DIRECTORY_MODE)


def _verify_existing_snapshot(destination: Path, files: dict[str, bytes]) -> None:
    descriptor = _open_directory_no_follow(destination.absolute())
    try:
        if _snapshot_inventory(descriptor) != set(files):
            raise ValueError(
...
def _snapshot_inventory(descriptor: int, prefix: str = "") -> set[str]:
    ...
    for name in os.listdir(descriptor):
        ...
        if stat.S_ISREG(metadata.st_mode):
            if stat.S_IMODE(metadata.st_mode) != _SNAPSHOT_FILE_MODE:
        ...
        try:
            files.update(_snapshot_inventory(child, relative))
        finally:
            os.close(child)
    return files
