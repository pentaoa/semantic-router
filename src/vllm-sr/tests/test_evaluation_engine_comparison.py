...
def _qualified_report(current: EvaluationReport) -> EvaluationReport:
    gates = tuple(
        (
...
def test_qualified_case_aligned_comparison_can_pass_and_fail(tmp_path: Path) -> None:
    ...
    passed = compare_reports(baseline, candidate, baseline_records, equal_records)
    failed = compare_reports(baseline, candidate, baseline_records, regressed_records)
    ...
    regret_failure = compare_reports(
...
def test_comparison_profiles_require_exact_canonical_treatment(
    ...
    assert compare_reports(recipe_baseline, recipe_candidate, [], []).candidate_run_id
    ...
    assert compare_reports(
        selector_baseline, selector_candidate, [], []
    ).candidate_run_id
    ...
    with pytest.raises(ValueError, match="selector_digest"):
        compare_reports(recipe_baseline, disguised_recipe, [], [])
    ...
    with pytest.raises(ValueError, match="policy_snapshot_digest"):
        compare_reports(selector_baseline, mixed_selector, [], [])
    ...
    assert compare_reports(
        adaptation_baseline, adaptation_candidate, [], []
    ).candidate_run_id
    ...
    with pytest.raises(ValueError, match="no independent server-owned"):
        compare_reports(agent_baseline, agent_candidate, [], [])
