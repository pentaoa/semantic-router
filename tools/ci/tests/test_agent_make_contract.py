...
class AgentMakeContractTests(unittest.TestCase):
    ...
    def test_local_smoke_uses_the_current_runtime_status_contract(self) -> None:
        smoke = target_block("agent-smoke-local")

        self.assertNotIn("Container Status: Running", smoke)
        self.assertIn("State[[:space:]]+Running", smoke)
        for service in ("Router", "Envoy", "Dashboard"):
            self.assertIn(
                f"{service}[[:space:]]+Running",
                smoke,
            )
