...
| `live-production-experiment`       | sealed randomized policy-arm assignments/exposures, rollout controls, and optional preference outcomes                                    | G8 uses operational safety receipts and risk UCB; G9 additionally requires complete propensity-qualified target/reference outcomes |
| `live-capacity`                    | repeated closed-loop load levels through the active Entrypoint under a frozen warmup, measurement, confidence, stability, and SLO contract | every request is phase/repetition/index bound; a server-reduced operating envelope and exact SLO headroom decide G7 without a proxy |
...
`declared-shift.server-live.v1;level=E4`. That narrow G4 statement concerns the
...
Gate disposition is `required`, `advisory`, `not_applicable`, or `waived`.
Gate verdict is `pass`, `fail`, `unavailable`, `not_applicable`, or `waived`.
...
| G7 Cost / latency / capacity | frozen typed SLO, at least two measured load levels, tail latency, error budget, minimum throughput, adjacent-level scaling, saturation, and headroom     | live Capacity creation requires the SLO and concurrency of at least two; `capacity.slo-envelope.v1` reduces the monotonic operating envelope, the server re-attests it from records, and G7 passes exactly when `capacity.slo_headroom >= 0`                                                                                                       |
...
Every run bundle is published atomically with private lifecycle metadata that
...
identities and email addresses are not stored in public run or lifecycle
responses. Start, cancel, delete, hold, release, and retention changes require
the owner or an administrator. Protected, held, baseline-referenced, and
Campaign-referenced evidence cannot be deleted or collected.

The local store enforces bounded per-owner run/byte quotas, a total physical
byte quota, and an audit byte bound. `GET /api/evaluation/v1/lifecycle/usage`
...
the exact eligible run, status, policy, evidence-reference, and byte snapshot;
a stale plan fails without deleting anything.

Lifecycle decisions are immutable, bounded, hash-chained records covering
create, start, cancel, hold, release, retention, delete, and garbage collection,
including denied authorization and protection decisions. Startup validates the
policy and complete audit chain. Global policy or audit corruption fails closed;
...
contract is fresh-store only and intentionally has no legacy migration path.
...
- strict JSON decoding, duplicate-key rejection, bounded IDs, record counts,
  file sizes, and event streams;
...
Worker-authored Markdown/HTML, records, grading labels, routing traces, private
lineage, failure cases, prompts, outputs, target origins, credentials, and
infrastructure identifiers are never public artifacts.
...
- `GET /api/evaluation/v1/campaigns/{id}` and `/decision`
...
manifest validation, execution, report rendering, comparison, and gate checks.
